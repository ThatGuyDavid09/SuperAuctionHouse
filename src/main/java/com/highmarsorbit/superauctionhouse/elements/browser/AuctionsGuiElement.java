...
public class AuctionsGuiElement extends BaseElement {
    private AuctionBrowserMenu.AuctionSortState sortState;
    public AuctionsGuiElement(char character, AuctionItem[] items, InventoryGui gui, AuctionBrowserMenu.AuctionSortState sortState) {
        super(character, gui);
    ...
    private List<AuctionItem> filterItems(AuctionItem[] items) {
        ...
        Stream<AuctionItem> typeFilteredItems = showableItems
                .filter(item -> switch (sortState.typeSort) {
                    case BOTH -> true;
                    case AUCTION_ONLY -> item.getAuctionType() == AuctionBrowserMenu.AuctionType.AUCTION;
                    case BIN_ONLY -> item.getAuctionType() == AuctionBrowserMenu.AuctionType.BUY_IT_NOW;
                });
